(() => {
      ...
      72: (n) => {
        ...
        function t(n, t) {
          for (var o = {}, i = [], c = 0; c < n.length; c++) {
            ...
            if (-1 !== u) e[u].references++, e[u].updater(b);
              ...
              var g = a(b, t);
              (t.byIndex = c),
                e.splice(c, 0, { identifier: s, updater: g, references: 1 });
            }
            i.push(s);
          }
          return i;
        ...
        n.exports = function (n, a) {
          ...
          return function (n) {
            ...
            for (var l = t(n, a), d = 0; d < o.length; d++) {
              var p = r(o[d]);
  ...
  let v = { currentOp: "0", operation: "", prevOperation: "" };
  function x(n) {
    v = { ...v, ...n };
  }
  function m() {
    let n = 0,
      e = parseFloat(v.prevOperation.replaceAll(",", ".")),
      r = parseFloat(v.currentOp.replaceAll(",", "."));
    if (!isNaN(e) && !isNaN(r)) {
      switch (v.operation) {
        case "+":
          n = e + r;
      ...
      x({
        currentOp: n % 1 == 0 ? n.toFixed(0) : n.toFixed(5),
        operation: "",
        prevOperation: "",
      });
    }
  ...
  function y() {
    let n = document.getElementById("result"),
      ...
      r = v.currentOp;
    v.prevOperation.length > 20 && (e.style.fontSize = "16px"),
      r.length > 15
        ? (n.style.fontSize = "16px")
        : r.length > 8
          ? (n.style.fontSize = "26px")
          : (console.log(r), (n.style.fontSize = "48px")),
      (n.innerText = v.currentOp),
      (e.innerText = v.prevOperation + v.operation);
  }
  window.addEventListener("load", () => {
    document.getElementById("loading").style.display = "none";
    let n = document.querySelectorAll("[aria-n]"),
      e = document.querySelectorAll("[aria-o]"),
      r = document.querySelector("[aria-equals]"),
      t = document.querySelector("[aria-ac]");
    document.querySelector("[aria-plusMinus]").addEventListener("click", () => {
      x({ currentOp: String(-1 * parseFloat(v.currentOp)) }), y();
    }),
      t.addEventListener("click", () => {
        x({ currentOp: "0", operation: "", prevOperation: "" }), y();
      }),
      n.forEach((n) => {
        n.addEventListener("click", (n) => {
          var e;
          ("," === (e = n.currentTarget.innerText) &&
            v.currentOp.indexOf(",") > -1) ||
            ("0" === v.currentOp && "," != e && x({ currentOp: "" }),
            "0" === v.currentOp && "," == e && x({ currentOp: "0" }),
            (v.currentOp + e).length > 25 ||
              ((v.currentOp += e), x({ currentOp: v.currentOp }))),
            y();
        });
      }),
      e.forEach((n) => {
        n.addEventListener("click", (n) => {
          ...
          (e = n.currentTarget.innerText),
            "" !== v.currentOp &&
              ("" !== v.prevOperation && m(),
              x({ operation: e, prevOperation: v.currentOp, currentOp: "" })),
            y();
        });
      }),
